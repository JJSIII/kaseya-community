...
(function ($) {
	$.fn.widgetSelector = function (options) {
		...
		settings.widgetList.bind('change', raiseChange);
		settings.themeList.bind('glowChange', raiseChange);

		settings.themeList.bind('glowChange', function () {
...
(function($){
	...
	$.fn.containerSelector = function(options) {
		...
		selector
			.undelegate(settings.pager + ' a','click')
			.delegate(settings.pager + ' a','click', function(e,data){
				e.preventDefault();
				var pageIndex = parseInt($(this).attr('data-page'),10);
				loadPageOfContainers(pageIndex);
				selectAll.find(settings.inputs).removeAttr('checked');
				options.empty();
				raiseChange();
			});
		...
		if(!liveBound) {
			liveBound = true;
			options.delegate(settings.inputs,'change',function(e,data){
				raiseChange();
			});
			selector.delegate(settings.allSelector,'change',function(e,data){
				if($(this).is(':checked')) {
					selector.find(settings.inputs).attr('checked','checked');
				} else {
					selector.find(settings.inputs).removeAttr('checked');
				}
				raiseChange();
			});
		}
...
(function($){
	...
	$.fn.replacementWidgetConfigurator = function(options) {
		...
		return this.each(function () {
			...
			if(!bound) {
				bound = true;
				$(document).bind('configuredContentFragment', function(e,data){
			...
			$(this).bind({
				targetWidgetSelection: function(e, data) {
					...
					if(data !== null && !nullOrEmpty(data.widget) && !nullOrEmpty(data.theme)) {
						...
						context.elements.containerSelector.find('input:checked').removeAttr('checked');
