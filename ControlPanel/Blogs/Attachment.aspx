<%@ Page language="c#" Codebehind="Attachment.aspx.cs" AutoEventWireup="false" Inherits="Telligent.Evolution.ControlPanel.Blogs.AttachmentPage" MasterPageFile="~/ControlPanel/Masters/Modal.master" %>
<%@ Import Namespace="Telligent.Evolution.Components" %>

<asp:Content ContentPlaceHolderId="bcr" runat="server">
	<TEControl:JQuery runat="server" />
	<TEBlog:UploadWeblogPostAttachmentForm id="AttachmentForm" runat="server" 
		...
		<FormTemplate>
			...
			<fieldset class="field-list">
				...
				<ul class="field-list">
					...
					<li class="field-item">
						<span class="field-item-input">
							<asp:LinkButton ID="Save" runat="server" OnClientClick="return validateForm(true);" CssClass="internal-link upload-file disabled"><span></span><TEControl:ResourceControl runat="server" ResourceName="Save" /></asp:LinkButton>
	...
	</TEBlog:UploadWeblogPostAttachmentForm>
...
<script type="text/javascript">
	...
	$(document).ready(function(e) {
		...
		if (chkUpload) chkUpload.click(function(e) { chkUploadClick(); });
		if (chkLink) chkLink.click(function(e) { chkLinkClick(); });

	});

	function uploadError() {
		isUploaded = false;
		$('.field-item.file .field-item-validation').html('<%= JavaScript.Encode(Telligent.Evolution.Components.ResourceManager.GetString("UploadAttachment_ErrorUploadingFile")) %>').show();
	}

	function uploadComplete() {
		isUploaded = true;
		if (chkLink) chkLink.attr("checked", false);
	...
	function validateForm(submit) {
	    ...
	    if (isLocal) {
		    ...
		    if (isUploaded) {
				...
				$('.field-item.file .field-item-validation').html('<%= JavaScript.Encode(Telligent.Evolution.Components.ResourceManager.GetString("UploadAttachment_MissingFile")) %>').show();
	    }
	    else if (isRemote) {
		    $('.field-item.file .field-item-validation').hide();
		    if (urlBox.val().length > 0 && urlBox.val().toLowerCase().indexOf('http') == 0) {
				...
				$('.field-item.link .field-item-validation').html('<%= JavaScript.Encode(Telligent.Evolution.Components.ResourceManager.GetString("UploadAttachment_MissingURL")) %>').show();
		    }
	    }
		if (isValid) {
			if (submit) {
