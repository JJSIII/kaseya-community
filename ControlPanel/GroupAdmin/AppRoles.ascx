<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="AppRoles.ascx.cs" Inherits="Telligent.Evolution.Web.ControlPanel.GroupAdmin.AppRoles" %>
...
<table cellpadding="3" cellspacing="0" border="0" width="500">
				...
				<tr>
					...
					<td>
						<div style="padding-right:8px">
							<p><input id="moveRight" type="button" value=" > " onclick="if (document.images) copySelected(<%=listAvailablePermissions.ClientID %>,<%=listGrantedPermissions.ClientID %>)"></p>
							<p><input id="moveLeft" type="button" value=" < " onclick="if (document.images) copySelected(<%=listGrantedPermissions.ClientID %>,<%=listAvailablePermissions.ClientID %>)"></p>
							<p><input id="moveAllRight" type="button" value=">>" onclick="if (document.images) copyAll(<%=listAvailablePermissions.ClientID %>,<%=listGrantedPermissions.ClientID %>)"></p>
							<p><input id="moveAllLeft" type="button" value="<<" onclick="if (document.images) copyAll(<%=listGrantedPermissions.ClientID %>,<%=listAvailablePermissions.ClientID %>)"></p>
						</div>
					</td>
					<td>
						<i><CP:ResourceControl ID="ResourceControl2" runat="server" ResourceName="CP_GroupAdmin_GrantedPerms" />:</i><br />
						<select runat="server" ID="listGrantedPermissions" name="listGrantedPermissions" class="RolesListBox" multiple="true" Width="200px" Height="250px" />
					</td>
				</tr>
			</table>
		</td>
	...
	<tr>
		<td colspan="2"><CP:ResourceButton ID="Save" ResourceName="Save" /></td>
	</tr>
</table>