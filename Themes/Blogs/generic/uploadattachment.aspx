<%@ Page EnableViewState="false" Language="C#" AutoEventWireup="true" Inherits="Telligent.Evolution.Blogs.Controls.CSBlogThemePage" MasterPageFile="~/themes/generic/Common/modal.Master" %>

<asp:Content ContentPlaceHolderID="content" runat="server">

	<TEControl:JQuery runat="server" />

	<TEControl:Title runat="server" ResourceName="UploadAttachment_Title" />
...
<script type="text/javascript">
	...
	function uploadError() {
		isUploaded = false;
		$('.field-item.file .field-item-validation').html('<%= Telligent.Evolution.Components.ResourceManager.GetString("UploadAttachment_ErrorUploadingFile") %>').show();
	}
	...
	function validateForm(submit) {
	    ...
	    if (isLocal) {
		    ...
		    if (isUploaded) {
			    ...
			    $('.field-item.file .field-item-validation').html('<%= Telligent.Evolution.Components.ResourceManager.GetString("UploadAttachment_MissingFile") %>').show();
	    }
	    else if (isRemote) {
		    $('.field-item.file .field-item-validation').hide();
		    if (urlBox.val().length > 0 && urlBox.val().toLowerCase().indexOf('http') == 0) {
			    ...
			    $('.field-item.link .field-item-validation').html('<%= Telligent.Evolution.Components.ResourceManager.GetString("UploadAttachment_MissingURL") %>').show();
		    }
	    }
		if (isValid) {
			if (submit) {
