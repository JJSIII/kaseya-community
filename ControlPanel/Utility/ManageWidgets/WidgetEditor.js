...
(function ($) {
	...
	setupEditorSettings = function (context) {
		...
		if (editorSettings.highlightLine) {
			context.elements.editorHighlightLineInput.attr('checked', 'checked');
		} else {
			context.elements.editorHighlightLineInput.removeAttr('checked');
		}

		if (editorSettings.showGutter) {
			context.elements.editorGutterToggleInput.attr('checked', 'checked');
		} else {
			context.elements.editorGutterToggleInput.removeAttr('checked');
		}

		if (editorSettings.showInvisibles) {
			context.elements.editorShowInvisiblesInput.attr('checked', 'checked');
		} else {
			context.elements.editorShowInvisiblesInput.removeAttr('checked');
		}
		...
		context.elements.editorSettingsFieldset
			.bind('mouseleave', function (e, data) {
	...
	setupAction = function (context) {
        ...
        context.elements.exportLink.bind('click', function(e) {
            prepareForSave(context);
        });
		// handle save button clicks
		context.elements.saveLink.bind('click', function (e) {
			e.preventDefault();
			save(context);
		});
		// handle save-as button clicks
		context.elements.saveAsLink.bind('click', function (e) {
			e.preventDefault();
		...
		context.elements.saveForThemeLink.bind('click', function (e) {
			e.preventDefault();
		...
		context.elements.themeSelect.bind('change', function (e) {
		...
		context.elements.deleteLink.bind('click', function (e) {
			e.preventDefault();
			var confirmText;
		...
		context.elements.revertLink.bind('click', function (e) {
			e.preventDefault();
		...
		context.elements.addFileLink.bind('click', function (e) {
			// add a new file tab
			addEditableFileTab();
			return false;
		});
		...
		$(context.elements.noAttachmentsMessageUploadLink).bind('click', function () {
			context.elements.uploadLink.click();
		});
		context.elements.helpLink.bind('click', function () {
	...
	setupHotKeys = function (context) {
		$(document).bind('keydown', function (e) {
			...
			if (e.ctrlKey && e.which == 83) {
				e.preventDefault();
				context.elements.saveLink.click();
				save(context);
			}
		});
	},
	...
	setupUi = function (context) {
		...
		context.parent
				.bind('fileTabCreated', function (e, data) {
					activateEditableText($(e.target).find('textarea'));
				})
				.bind('fileNameChanged', function (e, data) {
					if (data === null || data === '') {
				...
				.bind('nonEditableFileDeleted', function (e, data) {
				})
				.bind('nonEditableFileAdded', function (e, data) {
				...
				.bind('noNonEditableFiles', function (e, data) {
					context.elements.noAttachmentsMessage.show();
				})
				.bind('textModeChange', function (e, data) {
				...
				.bind('editorSettingsChanged', function (e, data) {
	...
	$.extend($.fn.widgetEditor, {
		defaults: {
			...
			selectors: {
				...
				noAttachmentsMessage: 'tr.NoAttachments',
				noAttachmentsMessageUploadLink: 'tr.NoAttachments a.attach',
				tabLinkTables: 'table',
