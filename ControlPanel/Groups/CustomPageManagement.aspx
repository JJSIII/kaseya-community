<%@ Page Language="C#" AutoEventWireup="true" EnableViewState="false" CodeBehind="CustomPageManagement.aspx.cs" Inherits="Telligent.Evolution.ControlPanel.Groups.CustomPageManagement" MasterPageFile="~/ControlPanel/Masters/Groups.master" %>
...
<asp:Content ContentPlaceHolderId="TaskRegion" runat="Server">
    ...
    function redirectSearch() {
        ...
        var customPageUrl = '<%= Globals.FullPath(GroupUrls.Instance().CustomPageManagement(CurrentGroup)) %>'
        ...
        <tr>
	        ...
	        <td align="right">
	            <div class="PanelSaveButton">
	                <a href="#" onclick="javascript:Telligent_Modal.Open('<%# GroupUrls.Instance().ConfigureCustomPage(CurrentGroup) %>', 380, 350, refresh);" class="CommonTextButton"><CP:ResourceControl runat="server" ResourceName="CP_Add_Page" /></a>
	            </div>
	        </td>
	    </tr>
	</table>
         ...
         <TEControl:WrappedRepeater runat="server" ID="PagesRepeater" ShowHeaderFooterOnNone="false">
            ...
            <ItemTemplate>
                <tr>
	                <td class="CommonListCellLeftMost">
	                    <a href='<%# GroupUrls.Instance().CustomPage(CurrentGroup, Eval("Name").ToString()) %>'><%# Eval("Title") %></a>
	                </td>
	                <td class="CommonListCellLeftMost">
	                    <a href='<%# GroupUrls.Instance().CustomPage(CurrentGroup, Eval("Name").ToString()) %>'>/<%# Eval("Name") %>.aspx</a>
	                </td>
	                <td class="CommonListCell">
	                    <CP:ModalLink runat="server" Visible='<%# Eval("ThemeContextID").ToString() != "-1" %>' ResourceName="Rename" Height="320" Width="550" callback="refresh" Url='<%# "ConfigureCustomPage.aspx?GroupID=" + CurrentGroup.ID + "&PageName=" + Eval("Name").ToString() %>' CssClass="CommonTextButton" />
	                </td>
	                <td class="CommonListCell">
	                    <a href='<%# GroupUrls.Instance().EditFragmentPage(CurrentGroup, Eval("Name").ToString(), true, -1, false, true, Globals.FullPath(GroupUrls.Instance().CustomPageManagement(CurrentGroup))) %>' class="CommonTextButton"><TEControl:ResourceControl runat="server" ResourceName="Layout" /></a>
	                </td>
	                <td class="CommonListCell">
	                    <CP:ModalLink runat="server" Visible='<%# Eval("ThemeContextID").ToString() != "-1" %>' ResourceName="Copy" Height="320" Width="550" callback="refresh" Url='<%# "CopyCustomPage.aspx?PageName=" + Eval("Name").ToString() + "&GroupID=" + CurrentGroup.ID %>' CssClass="CommonTextButton" />
	                </td>
	                <td class="CommonListCell">
            ...
            <AlternatingItemTemplate>
                <tr class="AltListRow">
	                ...
	                <td class="CommonListCell">
	                    <a href='<%# GroupUrls.Instance().EditFragmentPage(CurrentGroup, Eval("Name").ToString(), true, false, Globals.FullPath(GroupUrls.Instance().CustomPageManagement(CurrentGroup))) %>' class="CommonTextButton"><TEControl:ResourceControl runat="server" ResourceName="Layout" /></a>
	                </td>
	                <td class="CommonListCell">
	                    <CP:ModalLink runat="server" Visible='<%# Eval("ThemeContextID").ToString() != "-1" %>' ResourceName="Copy" Height="320" Width="550" callback="refresh" Url='<%# "CopyCustomPage.aspx?PageName=" + Eval("Name").ToString() + "&GroupID=" + CurrentGroup.ID %>' CssClass="CommonTextButton" />
	                </td>
            ...
            <NoneTemplate>
                <div class="CommonMessageWarning">
                    <TEControl:ConditionalContent runat="server">
                        ...
                        <TrueContentTemplate>
                            <CP:ResourceControl runat="server" ResourceName="CP_Groups_CustomPageManagement_NoSearchPages" />
                        </TrueContentTemplate>
                        <FalseContentTemplate>
                            <CP:ResourceControl runat="server" ResourceName="CP_Groups_CustomPageManagement_NoPages" />
                        </FalseContentTemplate>
    ...
    <TEControl:PlaceHolder runat="server" ID="ManageContentPages_RootGroup" Visible="false">
        <ContentTemplate>
            <div class="CommonMessageWarning">
                <CP:ResourceControl runat="server" ResourceName="CP_Settings_ConfigureCustomPage_RootGroup" />
                <TEControl:SiteUrl runat="server" UrlName="settings_ControlPanel_CustomPageManagement" 
					ResourceName="CP_Settings_CustomPageManagement_SiteContentPages" ResourceFile="ControlPanelResources.xml" />.
            </div>
        </ContentTemplate>
    </TEControl:PlaceHolder>
</asp:Content>
